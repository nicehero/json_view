...
#ifndef NICEHERO_DETAIL_WRITE_NUMBER_HPP
#define NICEHERO_DETAIL_WRITE_NUMBER_HPP
...
namespace nicehero { 
	...
	*/
	...
	inline bool round_weed(char *buffer, int len,
	...
	inline bool digit_gen(diy_fp_t Wm, diy_fp_t W, diy_fp_t Wp,
    ...
    void fill_exponent(int K, Result& result)
    {
        ...
        {
            nicehero::from_integer(K, result);
        }
    }
    ...
    bool dtoa_scientific(double val, char decimal_point, Result& result)
    {
        ...
        nicehero::to_double_t to_double_;
    ...
    bool dtoa_general(double v, char decimal_point, Result& result, std::true_type)
    {
        ...
        if (nicehero::grisu3(u, buffer, &length, &k))
        {
            if (std::signbit(v))
            {
                result.push_back('-');
            ...
            nicehero::prettify_string(buffer, length, k, -4, std::numeric_limits<double>::max_digits10, result);
            return true;
        }
        else
        {
    ...
    bool dtoa_fixed(double v, char decimal_point, Result& result, std::true_type)
    {
        ...
        if (nicehero::grisu3(u, buffer, &length, &k))
        {
            ...
            nicehero::prettify_string(buffer, length, k, std::numeric_limits<int>::lowest(), (std::numeric_limits<int>::max)(), result);
            return true;
        }
        else
        {
